...
class TestGame(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertIsInstance(self.game.renderer, MagicMock)
        self.assertIsInstance(self.game.input_handler, MagicMock)
        self.assertFalse(self.game.paused)
        self.assertFalse(self.game.game_over)

    def test_restart(self):
        # Change state        self.game.paused = True
    ...
    def test_run_loop_processes_input_and_moves(self, mock_sleep):
        ...
        self.game.input_handler.get_key.side_effect = input_sequence
        ...
        center_x = GRID_WIDTH // 2
        center_y = GRID_HEIGHT // 2        initial_body = [(center_x, center_y), (center_x-1, center_y), (center_x-2, center_y)]
        self.game.snake.body = initial_body
        self.game.snake.direction = DIRECTION_RIGHT

        # Place food ahead of snake so it will be eaten
        food_ahead = (center_x + 1, center_y)
        self.game.food.position = food_ahead        try:
            self.game.run()
    ...
    def test_run_loop_grows_when_eating_food(self, mock_sleep):
        ...
        self.game.snake.direction = DIRECTION_RIGHT
        food_ahead = (center_x + 1, center_y)
        self.game.food.position = food_ahead        self.game.run()

        # Snake should have grown (length 4) because it ate food        self.assertEqual(self.game.snake.get_length(), 4)
    ...
    def test_run_loop_detects_self_collision(self, mock_sleep):
        ...
        self.game.snake.direction = DIRECTION_UP  # (0,-1)
        # Place food nowhere near to avoid eating        self.game.food.position = (0,0)
    ...
    def test_run_loop_handles_wrap_around(self, mock_sleep):
        ...
        self.game.snake.body = [(GRID_WIDTH-1, 10), (GRID_WIDTH-2, 10), (GRID_WIDTH-3, 10)]
        self.game.snake.direction = DIRECTION_RIGHT
        # Place food at left edge same row to be eaten after wrap
        food_position = (0, 10)
        self.game.food.position = food_position        self.game.run()
    ...
    def test_run_loop_prevents_reversing_direction(self, mock_sleep):
        ...
        # Snake moving right        self.game.snake.direction = DIRECTION_RIGHT
        ...
        self.assertEqual(self.game.snake.direction, DIRECTION_RIGHT)

        # Now try to set direction to up (should work)
        self.game.input_handler.get_key.return_value = 'up'
        self.game.run()
        self.assertEqual(self.game.snake.direction, DIRECTION_UP)
    ...
    def test_run_loop_restarts_on_r_key(self, mock_sleep):
        ...
        # Should have reset        self.assertFalse(self.game.paused)
        self.assertFalse(self.game.game_over)
        self.assertNotEqual(self.game.snake, old_snake)
        self.assertNotEqual(self.game.food, old_food)
